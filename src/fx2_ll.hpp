...
/*
 * Copyright (c) 2021-2026 Gwenhael Goavec-Merou
 */
...
class FX2_ll {
 public:
	 ...
	 */
	FX2_ll(uint16_t uninit_vid, uint16_t uninit_pid, uint16_t vid,
	...
	/*!
	 * \brief Select the first usable bulk IN/OUT endpoint pair for an interface.
	 *
	 * Some Xilinx Platform Cable USB variants re-enumerate correctly after FX2
	 * firmware load, but Windows/libusbK/WinUSB may expose a different alternate
	 * setting or endpoint pair than the hard-coded Linux path. This helper scans
	 * the active configuration descriptor, tries the preferred alternate setting
	 * first, then falls back to another alternate setting with one bulk OUT and
	 * one bulk IN endpoint.
	 */
	bool select_bulk_endpoints(const int if_num, uint8_t &out_endpoint,
			uint8_t &in_endpoint, const int preferred_alt_setting = 1);
	 ...
	 */
	int write_ctrl(uint8_t bRequest, uint16_t wValue, uint8_t *buff,
			uint16_t len, uint16_t wIndex = 0x0000);
	 ...
	 * \return false when transfer fails, true otherwise
	 */
	int read_ctrl(uint8_t bRequest, uint16_t wValue, uint8_t *buff,
			uint16_t len, uint16_t wIndex = 0x0000);

	/*!
	 * rief clear a stalled endpoint
	 * \param[in] endpoint: endpoint address, with or without direction bit
	 * 
eturn true on success
	 */
	bool clear_halt(uint8_t endpoint);

	/*!
	 * rief reset the USB device handle
	 * 
eturn true on success
	 */
	bool reset_device();
