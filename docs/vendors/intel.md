...
![max10 internal flash memory structure](../figs/max10_flash-memory.png)
...
Depending on the max10 configuration mode (see picture), it's possible to extend *User Flash Memory* area by using <span class="title-ref">CFM2</span> and <span class="title-ref">CFM1</span>. This is not the default behavior and user must explictly change this by using <span class="title-ref">--flash-sector</span> argument:
