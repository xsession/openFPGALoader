...
6. run the control-transfer JTAG implementation repeatedly without requiring a
   reconnect.
...
### 4. Use control transfers by default on Windows

On this cable/firmware/WinUSB combination, the accelerated XPCU `0xA6` request
times out and stalls endpoint zero. Windows therefore defaults to the reliable
...
| `OPENFPGALOADER_XPCU_ACCELERATED=1` | Opt into experimental accelerated Windows transfers |
